...
exports.buscarDisponibles = (req, res) => {
  ...
  Reserva.findAll({
    ...
    .then((reservas) => {
      const habitacionesOcupadas = reservas.map(
        (reserva) => reserva.id_habitacion
      );
      ...
      Habitacion.findAll()
        .then((habitaciones) => {
          ...
          const habitacionesDisponibles = habitaciones.filter((habitacion) => {
            return (
              !habitacionesOcupadas.includes(habitacion.id) &&
          ...
          const habitacionesSinTimestamps = habitacionesDisponibles.map(
            (habitacion) => {
        ...
        .catch((err) => {
          res.status(500).send({
            message:
              err.message ||
              "Ha ocurrido un error al buscar habitaciones disponibles.",
          });
        });
    })
    .catch((err) => {
